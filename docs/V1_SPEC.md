...
- background monitor runtime 的長時間穩定性
- 睡眠恢復補掃、通道節流持久化與更完整的錯誤映射
- watch 的啟用 / 暫停 / 手動立即檢查 GUI
...
V1 的正式建立流程以專用 Chrome 分頁為唯一主入口：

1. 使用者從專用 Chrome 已開啟的 `ikyu` 分頁中選擇一頁
2. 系統讀取該頁面內容並建立 `search_draft`
3. 系統解析候選項並自動比對目前分頁中的預填條件
4. 使用者確認或改選
5. 產生最終 `watch_target`
