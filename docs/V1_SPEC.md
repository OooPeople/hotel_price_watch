...
1. 使用者貼入 `ikyu` 飯店頁或方案頁 URL。
2. 系統解析 URL，建立查詢條件。
3. 系統抓取頁面，列出可監看的房型方案候選。
4. 使用者選定要追蹤的方案。
5. 系統建立 watch item。
6. 背景排程定期抓取與比對。
7. 價格下降或狀態改變時發出通知。
...
- 入口 A：貼入一般飯店頁 URL，系統查詢候選房型方案
- 入口 B：貼入已包含 `cid`、`rm`、`pln` 的精確 URL，系統直接預填日期與已選方案
...
1. 從 HTML 中的 SSR 可見價格區塊解析
2. 從 Nuxt hydration 資料解析 `hotel / room / plan / amount` 相關欄位
3. 如果上述失敗，再考慮 browser fallback
...
- 預設 jitter 為 `±10%`
- 預設最大並行數為 `2` 到 `3`
- timeout / parse failure / `429` 需採 backoff 與自動重試
- `429` 使用較長 backoff
- `403` 暫停該 watch item，並在 UI 顯示需人工介入
- 不因單次錯誤立即停用 watch item
- `parse_failed` 單次失敗只記錄；連續 `3` 次後標記為 degraded 並通知一次
- 電腦從睡眠恢復後應盡快補掃一次
- V1 不自動啟動 browser fallback
