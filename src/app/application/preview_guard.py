...
class PreviewAttemptGuard:
    ...
    _next_allowed_at: datetime | None = field(default=None, init=False)
    _cooldown_reason: str | None = field(default=None, init=False)

    def ensure_allowed(self, *, now: datetime | None = None) -> None:
        """若 preview 仍在冷卻中，直接阻止本次嘗試。"""
        current_time = now or datetime.now(UTC)
        if self._next_allowed_at is None or current_time >= self._next_allowed_at:
            return

        retry_after_seconds = max(
            0.0,
            (self._next_allowed_at - current_time).total_seconds(),
        )
        raise PreviewCooldownError(
            "目前 preview 仍在冷卻中；請稍後再重試，避免持續觸發 `ikyu` 風控。",
            diagnostics=(
                LookupDiagnostic(
                    stage="preview_rate_guard",
                    status="cooldown_active",
                    detail=self._cooldown_reason or "上一輪 preview 已進入冷卻。",
    ...
    def register_result(
        ...
        *,
        ...
        current_time = now or datetime.now(UTC)
        cooldown_seconds, reason = self._decide_cooldown(diagnostics)
        self._next_allowed_at = current_time + timedelta(seconds=cooldown_seconds)
    ...
    def _decide_cooldown(
        ...
        if any(
            ...
            return (
                ...
                "上一輪 preview 已命中 `ikyu` 阻擋頁或 403；先做較長冷卻。",
            )
